module Wautoma.SendInputBindings
...
let pressKey (code: int) =
    let mutable input = createPressInput code

    let result =
        NativeMethods.SendInput(
            uint32 1,
            &input,
            Marshal.SizeOf(typedefof<LPINPUT>)
        )

    ignore ()

let releaseKey (code: int) =
    let mutable input = createReleaseInput code

    NativeMethods.SendInput(uint32 1, &input, Marshal.SizeOf(input))
    |> ignore
